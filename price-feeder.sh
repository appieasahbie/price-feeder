...
read -r -p "Enter password to you main wallet: " 

printGreen "Creating wallet for price feeder"
echo | ojod keys add price_feeder_wallet 
printDelimiter
...
echo $WALLET_PASS | ojod tx bank send $MAIN_WALLET_ADDRESS $PRICEFEEDER_ADDRESS 1000000uojo --from $MAIN_WALLET_ADDRESS --chain-id ojo-devnet --fees 5000uojo --gas-adjustment 1.4 --gas auto -y

printGreen "Waiting 5 seconds..." && sleep 5
printGreen "Delegate price feeder responsibility"
echo $WALLET_PASS | ojod tx oracle delegate-feed-consent $MAIN_WALLET_ADDRESS $PRICEFEEDER_ADDRESS --from $MAIN_WALLET_ADDRESS --chain-id ojo-devnet --gas-adjustment 1.4 --fees 5000uojo --gas auto -y
